/**
 * TESTIFY CANVAS SYSTEM v1.1
 * ===========================
 * Ultra-premium drawing/writing system for YKS test solving
 * Features: pressure simulation, shape snapping (line / circle / rect / triangle),
...
(function () {
    ...
    class TestifyCanvas {
        ...
        tryRecognizeShape(strokeData) {
            ...
            const closedRatio = closedDist / (diag || 1);

            const perimeter = this.calculatePerimeter(pts);
            ...
            let recognized = null;
            const meta = { minX, maxX, minY, maxY, width, height, diag };

            // Açık şekiller – düz çizgi mi?
            if (closedRatio > 0.35) { // uçlar birbirine çok yakın değil
                const lineScore = this.getLineStraightnessScore(pts);
                const dominantAxis = (width > height ? width / (height || 1) : height / (width || 1));
                if (lineScore > 0.9 && dominantAxis > 2) {
                    recognized = 'line';
                }
            } else {
                // Kapalı şekiller
                ...
                const rStd = Math.sqrt(
                    radii.reduce((sum, r) => sum + Math.pow(r - rMean, 2), 0) / radii.length
                );
                const radiusUniformity = 1 - (rStd / (rMean || 1)); // 1'e yakınsa düzgün daire

                const corners = this.findCorners(pts, 0.25);
                const cornerCount = corners.length;

                if (radiusUniformity > 0.85 && circularity > 0.85 && aspectRatio > 0.8 && aspectRatio < 1.25) {
                    recognized = 'circle';
                    meta.cx = cx;
                    meta.cy = cy;
                    meta.radius = Math.min(width, height) / 2;
            ...
            // Yeni, temiz shape noktalarını üret
            const newPoints = this.buildShapePoints(recognized, meta, strokeData.points);
            ...
            this.showShapeHint(recognized, minX + width / 2, minY - 20);
        }
        ...
        buildShapePoints(shape, meta, originalPoints) {
            const { minX, maxX, minY, maxY } = meta;
            const pts = [];
            const pressure = 0.6;
            const push = (x, y) => pts.push([x, y, pressure]);
            ...
            if (shape === 'line') {
                ...
                for (let i = 0; i <= segments; i++) {
                    ...
                    const y = first[1] + (last[1] - first[1]) * t;
                    push(x, y);
                }
                ...
                const p1 = [(left + right) / 2, top];
                const p2 = [right, bottom];
                const p3 = [left, bottom];

                const corners = [p1, p2, p3, p1];
